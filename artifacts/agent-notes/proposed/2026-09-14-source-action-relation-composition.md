...
verification needed to supply that configuration. The full overnight training
experiment has not been started. This note remains proposed; no exact-revision
acceptance, product commit, lifecycle transition or remote publication is implied.
...
- Dataset: pinned annotation revision and split consumed by PreparedCorpus;
  local index `artifacts/indexes/beatmap_index_4k.parquet`; 2,048 novel training
  song groups, 128 local validation groups and two 128-event windows per chart.
  Annotation prediction contexts have 4–256 events. The existing connected
  metadata-group exclusions are applied before training and human fitting.
- Evaluation slice: 64 hash-selected groups from combined validation contexts,
  up to two targets per feasible scale/group. Exact source hashes, Parquet
  allocation, population identity and block manifest are written before updates.
...
- Files: source_action_modeling composition/experiment/structural_probe modules,
  shared encoder helper extraction, evaluation callbacks, population arguments,
  packaged preset and focused tests. Read-only context: source-action objective
...
- Updates: 8 blocks times three views, AdamW 3e-4, weight decay 1e-4, gradient
  cap 1; at most 20,000 updates or 12,600 training seconds per seed, minimum 1,000.
...
- Confounders: support/optimization changes, decoder prefix use, imperfect song
  ...
  correlated structural statistics. Factor holdouts apply to the diagnostic
