# Ensomi：给定 time skeleton 的因果谱面续写
...
[R1] ensomi-model，`docs/formulation/notation.md`，公开提交 `a7ab19f26cd8dd05480c8376060aaced1af4f2a7`：完整 rows、四动作合法性、LN closure、committed-prefix 合同。

[R2] 同提交，`src/ensomi_model/research/source_action_modeling/{local_representation,representation,time_basis,model,actions}.py`：现有双向局部支持、pace、ActionReader/GRU 与 action schema。本文提出的 causal path 不是这些代码已经实现的功能。

[R3] 同提交，`src/ensomi_model/configs/hydra/source_action_composition.yaml`：现有 MPS 环境与资源 guard 起点。
