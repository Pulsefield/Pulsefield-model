---
name: ensomi-trim-cot-leakage
description: Audit or fix Ensomi prose whose viewpoint depends on an authoring session, chat, review, branch, uncommitted plan, or unexplained local evidence. Use for dead session citations, publication narration, reviewer choreography, derivation transcripts, planning residue, and indexical version language. Route general clarity, documentation coverage, and prose structure to ensomi-prose-standard.
---
...
Preserve every relevant actor, condition, ordering rule, modality, negative guarantee, ownership fact, failure, consequence, measurement, and uncertainty. Apply [`ensomi-prose-standard`](../ensomi-prose-standard/SKILL.md) for broader editorial judgment after the leakage-specific audit.
