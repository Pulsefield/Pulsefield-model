...
- Use [`ensomi-prose-standard`](../ensomi-prose-standard/SKILL.md) when simplifying comments or documentation.
- Use [`ensomi-archive-agent-notes`](../ensomi-archive-agent-notes/SKILL.md) only when lifecycle-managed decision records are actually in scope.
...
- Exclude `ref-proj/` unless comparison work is explicit. A reference project can suggest a question but cannot establish what Ensomi should remove.
...
Do not relabel documented constraints as accidental complexity. Treat these Ensomi boundaries as intentional unless new evidence defeats their rationale:

- Packaged YAML under `src/ensomi_model/configs/` is canonical for mapper training and inference configuration.
...
When the user requests breadth or many candidates, divide the survey among independent domains and require evidence from each. Useful Ensomi domains include:
...
- **Production:** `src/ensomi_model/`, packaged YAML and JSON, import and runtime entrypoints, model-bundle dispatch, and operator commands.
- **Non-production:** unit tests, curated docs, comments, and test-only fixtures.
- **Ambiguous:** notebooks, `src/ensomi_model/evals/`, repo-root `configs/`, local scripts, compatibility loaders, and smoke paths. Read their invocation and ownership before assigning them.
