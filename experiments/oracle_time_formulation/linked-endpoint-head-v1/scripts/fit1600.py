...
def main():
    ...
    draws=[[rng.randrange(len(train)) for _ in range(32)] for _ in range(1600)]
    save(OUT/'manifest.json',dict(feature_manifest_sha256=file_digest(FEATURES/'manifest.json'),seed=SEED,
         train_cases=len(train),validation_cases=len(validation),updates=1600,batch=32,lr=.001,weight_decay=.01,clip=1.,
    ...
    original=json.loads((OWNER/'fit/manifest.json').read_text())
    assert draws[:400]==original['shared_draws']
    history=[]
    with (OUT/'updates.jsonl').open('x') as stream:
        for step,indices in enumerate(draws,1):
            ...
            if step==400:
                for arm,model in models.items():
                    old=torch.load(OWNER/'fit'/f'{arm}.pt',map_location='cpu',weights_only=True)['model']
                    assert all(torch.equal(value,old[key]) for key,value in model.state_dict().items()), '400-step weight mismatch'
                save(OUT/'prefix400-parity.json',dict(status='exact',arms=['prior','context'],shared_draws=True))
