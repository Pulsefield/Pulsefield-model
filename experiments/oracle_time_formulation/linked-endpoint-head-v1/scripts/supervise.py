...
def main():
    mode=sys.argv[1]
    assert mode in ('extract','fit')
    destination=OWNER/('features' if mode=='extract' else 'fit')
    assert not destination.exists(), 'Inspect partial outputs; no blind restart or overwrite'
    limit=1800 if mode=='extract' else 1200
    script=Path(__file__).with_name(mode+'.py')
    ...
    result=dict(status='complete',mode=mode,seconds=time.monotonic()-started,
                readout_sha256=file_digest(destination/('manifest.json' if mode=='extract' else 'readout.json')))
