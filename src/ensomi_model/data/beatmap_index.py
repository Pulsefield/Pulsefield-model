...
from ensomi_model.osu_core.difficulty import calculate_mania_difficulties
from ensomi_model.osu_core.metadata import parse_osu_metadata
from ensomi_model.osu_core.timing import InvalidRedTimingError, MissingRedTimingError, require_red_timing_points
from ensomi_model.timing.rendering.dense_timing_v2 import DENSE_TIMING_V2_CHANNELS, DENSE_TIMING_V2_VERSION
...
def _format_command(argv: Sequence[str] | None, subcommand: str) -> str:
    ...
    return " ".join(
        shlex.quote(part)
        for part in ["python", "-m", "ensomi_model.data.beatmap_index", subcommand, *args]
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Ensomi beatmap indexes.")
