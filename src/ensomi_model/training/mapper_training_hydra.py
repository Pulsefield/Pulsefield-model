...
from ensomi_model.training.cli_legacy import reject_deprecated_legacy_training_flag
...
from ensomi_model.training.mapper_common import precompute_mapper_tuple_phase_b_control_teacher_cache
    ...
    *,
    ...
    if mapper_kind == "v2":
        if v2_runner is not None:
            return v2_runner
        from ensomi_model.training.mapper_v2 import run_mapper_v2_phase_b_training

        return run_mapper_v2_phase_b_training
    if mapper_kind == "v2_1":
        if v21_runner is not None:
            return v21_runner
        from ensomi_model.training.mapper_v2_1 import run_mapper_v2_1_phase_b_training
    ...
    for token in argv:
        if token.startswith("training/mapper=") or token.startswith("+training/mapper="):
            _raise_hydra_cli_usage(
                ...
                "use ensomi_model.training.mapper_training_hydra to select another mapper group",
            )
