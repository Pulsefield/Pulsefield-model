        ...
        for lane, is_open in enumerate(open_mask):
            ...
            if is_open:
                ...
                if open_start > current_ms:
                    raise ValueError(
                        f"open_start_ms cannot be after current_ms on lane {lane}: "
                        f"{open_start} > {current_ms}",
                    )
...
def transition_replay_state(
    ...
    *,
    ...
    if vocab.is_time_shift_token(token_id):
        ...
        if next_ms == int(write_end_ms) and not replay_state_matches_carry(next_state, ln_carry_out):
    ...
    raise ReplayError(f"unknown mapper tuple token id: {token_id}")
...
def _validate_boundary_carry_states(
    *,
    ...
    if int(ln_carry_out.current_ms) != int(write_end_ms):
        raise ValueError(f"ln_carry_out.current_ms must equal write_end_ms: {ln_carry_out.current_ms} != {write_end_ms}")
