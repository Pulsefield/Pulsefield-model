...
from .vocab import ACTION_TO_CODE, KEY_COUNT, LaneAction, MapperTupleVocab
...
class LNCloseAdapterOutput:
    ...
    def __iter__(self):
        ...
        yield self.event_bias
        yield self.time_shift_bias


class StatePriorAdapter(nn.Module):
    """State-only structured prior projected through EVENT token lane actions."""
    ...
    def __init__(
        ...
        *,
        ...
        _register_event_projection_buffers(self, vocab)
    ...
    def project_lane_action_bias(self, lane_action_bias: torch.Tensor) -> torch.Tensor:
        return _project_event_lane_action_bias(
            lane_action_bias,
            vocab_size=self.vocab.size,
            event_token_ids=self.event_token_ids,
            event_action_codes=self.event_action_codes,
        )
...
class LNCloseAdapter(nn.Module):
    """Context-aware lane-level close hazard head and constrained logit biases."""
    ...
    def forward(
        ...
        *,
        ...
        event_bias = self._project_close_event_bias(close_bias=close_bias, open_mask=open_mask)
    ...
    def _project_close_event_bias(self, *, close_bias: torch.Tensor, open_mask: torch.Tensor) -> torch.Tensor:
        batch_size, steps, _ = close_bias.shape
        event_bias = close_bias.new_zeros((batch_size, steps, int(self.event_token_ids.numel())))
        open_float = open_mask.to(dtype=close_bias.dtype)
        for lane in range(KEY_COUNT):
            action_codes = self.event_action_codes[:, lane].reshape(1, 1, -1)
            lane_close = close_bias[:, :, lane].unsqueeze(-1) * open_float[:, :, lane].unsqueeze(-1)
            close_token = action_codes == ACTION_TO_CODE[LaneAction.HOLD_END]
            keep_token = action_codes == ACTION_TO_CODE[LaneAction.NONE]
            event_bias = event_bias + torch.where(
                close_token,
                lane_close,
                torch.where(keep_token, -0.25 * lane_close, torch.zeros_like(lane_close)),
            )

        vocab_bias = close_bias.new_zeros((batch_size, steps, self.vocab.size))
        vocab_bias[:, :, self.event_token_ids] = event_bias
        return vocab_bias

    def _project_time_shift_bias(self, *, close_logits: torch.Tensor, open_mask: torch.Tensor) -> torch.Tensor:
        batch_size, steps, _ = close_logits.shape
        vocab_bias = close_logits.new_zeros((batch_size, steps, self.vocab.size))
...
def _project_event_lane_action_bias(
    lane_action_bias: torch.Tensor,
    *,
    vocab_size: int,
    event_token_ids: torch.Tensor,
    event_action_codes: torch.Tensor,
) -> torch.Tensor:
    ...
    batch_size, steps = lane_action_bias.shape[:2]
    event_bias = lane_action_bias.new_zeros((batch_size, steps, int(event_token_ids.numel())))
    for lane in range(KEY_COUNT):
        lane_bias = lane_action_bias[:, :, lane, :]
        action_index = event_action_codes[:, lane].reshape(1, 1, -1).expand(batch_size, steps, -1)
        event_bias = event_bias + lane_bias.gather(dim=-1, index=action_index)

    vocab_bias = lane_action_bias.new_zeros((batch_size, steps, int(vocab_size)))
    vocab_bias[:, :, event_token_ids] = event_bias
    return vocab_bias


def project_lane_action_bias_to_event_tokens(
    lane_action_bias: torch.Tensor,
    vocab: MapperTupleVocab,
) -> torch.Tensor:
    event_token_ids = torch.tensor(vocab.event_token_ids, dtype=torch.long, device=lane_action_bias.device)
    event_action_codes = torch.tensor(
        [
            [ACTION_TO_CODE[action] for action in vocab.decode_event(token_id)]
            for token_id in vocab.event_token_ids
        ],
        dtype=torch.long,
        device=lane_action_bias.device,
    )
    return _project_event_lane_action_bias(
        lane_action_bias,
        vocab_size=vocab.size,
        event_token_ids=event_token_ids,
        event_action_codes=event_action_codes,
    )
...
def project_ln_close_bias_to_tokens(
    *,
    ...
    event_token_ids = torch.tensor(vocab.event_token_ids, dtype=torch.long, device=close_logits.device)
    event_action_codes = torch.tensor(
        [
            [ACTION_TO_CODE[action] for action in vocab.decode_event(token_id)]
            for token_id in vocab.event_token_ids
    ...
    event_bias = close_logits.new_zeros((close_logits.shape[0], close_logits.shape[1], int(event_token_ids.numel())))
    for lane in range(KEY_COUNT):
        action_codes = event_action_codes[:, lane].reshape(1, 1, -1)
        lane_close = close_bias[:, :, lane].unsqueeze(-1) * open_float[:, :, lane].unsqueeze(-1)
        close_token = action_codes == ACTION_TO_CODE[LaneAction.HOLD_END]
        keep_token = action_codes == ACTION_TO_CODE[LaneAction.NONE]
        event_bias = event_bias + torch.where(
    ...
    ln_close_bias[:, :, event_token_ids] = event_bias
    time_shift_bias = close_logits.new_zeros((close_logits.shape[0], close_logits.shape[1], vocab.size))
    if float(skip_scale) != 0.0 and int(time_shift_token_ids.numel()) > 0:
...
def _register_event_projection_buffers(module: nn.Module, vocab: MapperTupleVocab) -> None:
    event_token_ids = torch.tensor(vocab.event_token_ids, dtype=torch.long)
    event_action_codes = torch.tensor(
        [
            [ACTION_TO_CODE[action] for action in vocab.decode_event(token_id)]
            for token_id in vocab.event_token_ids
    ...
    module.register_buffer("event_token_ids", event_token_ids, persistent=False)
    module.register_buffer("event_action_codes", event_action_codes, persistent=False)
