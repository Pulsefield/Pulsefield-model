...
from .program import HEADS, preview, row_support, tokens
...
def collate(example, model, program, controls, recovery, device):
    ...
    clocks = np.stack([program.states[i].clocks(t, duration) for i, t in zip(indices, x.timing_times.numpy())])
    ...
    for i, now in zip(row_indices, x.row_times.numpy()):
        ...
        rclocks.append(program.states[i].clocks(now, duration))
    ...
    return dict(base=replace(x, **{k: v.to(device) for k, v in vars(x).items()}),
        ...
        marks=tensor(marks[row_indices[:nrows]], torch.long), rows=base.targets.row_index.to(device))
